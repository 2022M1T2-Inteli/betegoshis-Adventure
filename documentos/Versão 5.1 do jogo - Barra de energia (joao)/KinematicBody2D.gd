...
var foto = preload("res://Action RPG Resources/Player/caradoBTG andando.png")
...
func get_input():
	...
	if Input.is_action_pressed("keyD") and Input.is_action_pressed("keyW") or irCimaDireita == true: #se apertar "D" e "W" simultaneamente
		...
		velocity.y += SPEED #                                #
		$AnimationPlayer.play("correndinho pra esquerda")
		$Camera2D/Celular/AbrirCelular/ProgressBar.value -= 0.01
...
func _process(_delta):
	...
	$Camera2D/Dinheiro/DinheiroLabel.text = "BT$  " + str(Global.money)
	$Camera2D/Celular/AbrirCelular/ProgressBar.value = Global.energy # energia x tempo
