...
func _on_nextfala_tio_pressed():
	...
	if Global.dia == 2:
		if contadorproximafalatio == 3:
			$TioClovisFalas/falastio.text = "Bom dia, " + str(Global.player_name) + "!" 
			$TioClovisFalas/nextfala_tio.visible = true
			$TioClovisFalas/sairFala.visible = true
		if contadorproximafalatio == 4:
			$ProfessoraFalas/IniciarAula/sprite_inicio_aula1.text = "Farei um strogonofe de Leão hoje, pode ir ao mercado e comprar frango e cogumelo, por gentileza?"
			$TioClovisFalas/nextfala_tio.visible = false
			$TioClovisFalas/sairFala.visible = true
			$TioClovisFalas/sairFala/Atemais2.text = "Claro!"
		if contadorproximafalatio == 4 and Global.frango == true and Global.cogumelo == true:
			$TioClovisFalas/falastio.text = "Obrigado, " + str(Global.player_name) + "! Farei comida agora, em uma horinha estará pronta."
			$TioClovisFalas/nextfala_tio.visible = false
			$TioClovisFalas/sairFala.visible = true
