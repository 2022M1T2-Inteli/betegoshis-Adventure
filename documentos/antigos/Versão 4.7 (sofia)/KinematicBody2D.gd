...
func _ready(): #iniciar a cena já ativa os comandos abaixo
	...
	self.position.x = -1932  #teleporta pra porta da facudlade
...
func fecharBotoes(botao, contador):
	botao.modulate.a = 0
	contador += 1
...
# Sair do Arcade 509 2207

func _on_TP_Sair_Arcade_body_entered(body):
	# Teleporta para fora do arcade
	self.position.x = 509
	...
	$Camera2D.limit_right = 2400
# Entrar no Arcade
func _on_Arcade_TP_body_entered(body):
...
func _on_FaculdadeTP_pressed(): #botão para teleportar para a faculdade dentro do celular
	...
	if irFaculdade == true and irTaxi == true: #se ativar a faculdade e o taxi
		...
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(money) #valida isso fodase
	...
	if irFaculdade == true and irOnibus == true: #se ativar a faculdade e o onibus
		...
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(money) #valida isso
		goToFaculdade = true #valida a ida para a faculdade
...
func _on_CasaTPad_pressed():
	...
	if irCasa == true and irBike == true: #se ativa casa e bicicleta
		money -= precoBike #paga
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(Global.money) #valida isso
		goToCasa = true #valida a ida para a casa
...
func _on_ArcadeTPad_pressed(): #botão para teleportar para a faculdade dentro do celular
	Global.money = money
	...
	if contadorArcadeTPButton % 2 == 1: #quando for impar
		irArcade = false #ir pra faculdade está desativado
		$Camera2D/Celular/AbrirCelular/CelularPanel/apps/MapaB/MapaI/ArcadeTPad.modulate.a = 0 #mostra as letras
	if irArcade == true and irTaxi == true: #se ativar a faculdade e o taxi
		money -= precoTaxi #paga
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(money) #valida isso fodase
		goToArcade = true #valida a ida para a faculdade
		
	if irArcade == true and irOnibus == true: #se ativar a faculdade e o onibus
		money -= precoOnibus #paga
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(money) #valida isso
		goToArcade = true #valida a ida para a faculdade
		
		
	if irArcade == true and irBike == true: #se ativa faculdade e bicicleta
		money -= precoBike #paga
		$Camera2D/Dinheiro/DinheiroLabel.text = "BT$"+str(money) #valida isso
		goToArcade = true #valida a ida para a faculdade
...
func _on_PagarBoleto_pressed(): # botão de "Pagar Boleto" na conversa do Guilherme
	if boleto == true: # se ele possui o boleto (falou com a secretária)
		...
		$Camera2D/Falas/GuilhermeFalas/sprite_botao_boleto.visible = false
...
func _on_Iniciar_Japinha_Falas_body_entered(_body): # botão atrás do 
	$Camera2D/Falas/JapinhaFalas/Sprite.visible = true
...
func _on_BotaoComprarComida_pressed():
	...
	if contadorBotaoComprarComida % 2 == 1:
		...
		$Camera2D/Falas/JapinhaFalas/Sprite.visible = false

		
	if contadorBotaoComprarComida % 2 == 0:
		$Camera2D/Falas/JapinhaFalas/Comidinhas.visible = false
		$Camera2D/Falas/JapinhaFalas/SairCvsaJapa.visible = true
		

func _on_ComprarComida1_pressed():
	Global.money = money
	money -= 5
	$Camera2D/Dinheiro/DinheiroLabel.text = "BT$   " + str(money)
