...
func _ready(): #iniciar a cena já ativa os comandos abaixo
	...
	#Teleporte para o quarto
	self.position.x = -2627 #teleporta pro quarto
	self.position.y = 2258 ##
	
	$"Camera2D/Dinheiro dele".text = str(money) #mostra o dinheiro inicial dele
...
func _on_Faculdade_TP_body_entered(_body): # vai pra faculdade
	irCimaDireita = false # para de andar para nordeste
	irCimaEsquerda = false #para de andar para noroeste
	irCima = true #anda pra norte
...
func _on_Professorabotao_pressed(): #quando a aula começa, ele centraliza nos slides teleportando
...
func _on_BancoTPad_pressed():
	...
	if goToBanco == true:
		yield(get_tree().create_timer(0.8), "timeout") #espera 0.8 sec
		self.position.x = -3240  #teleporta pra porta da facudlade
		self.position.y = -349##
		$Camera2D/Config/iPad/apps/MapaB/MapaI.visible = false #desaparece o acesso ao mapa
		$Camera2D/Config/iPad/apps/MapaB/CloseMAPButton.visible = false #desaparece o botao 
		$Camera2D/Controles.visible = true #aparecem os controles
...
func _on_Iniciar_Guilherme_Falas_body_entered(body):
	$Camera2D/Falas/GuilhermeFalas.visible = true
	$Camera2D/Controles.visible = false
	$Camera2D/Config.visible = false
	pararMovimento = true
...
func _on_Iniciar_Secretria_Falas_body_entered(body):
