...
func _on_PagarBoleto_pressed(): # botão de "Pagar Boleto" na conversa do Guilherme
	if boleto == true: # se ele possui o boleto (falou com a secretária)
		...
		$Notificacao.visible = true # aparece uma notificação de missão completa
...
func _on_BotaoComprarComida_pressed():
	...
	if contadorBotaoComprarComida % 2 == 1:
		...
		$Camera2D/Falas/JapinhaFalas/JapaFalasButton.visible = false
		$Camera2D/Falas/JapinhaFalas/SairCvsaJapa.visible = false
		$Camera2D/Falas/JapinhaFalas/BotaoComprarComida.text = "Voltar"
	if contadorBotaoComprarComida % 2 == 0:
		...
		$Camera2D/Falas/JapinhaFalas/BotaoComprarComida.text = "Boa! E agora?"

func _on_ComprarComida1_pressed():
	Global.money = money
	money -= 5
	$Camera2D/Dinheiro/DinheiroLabel.text = "BTK   " + str(money)
