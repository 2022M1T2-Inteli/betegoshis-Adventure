...
func _ready():
	$"Tela de Carregamento 1/AnimationPlayer".play("tela de carregamento")
	yield(get_tree().create_timer(2.6), "timeout")
	$"Tela de Carregamento 1".visible = false
	self.position.x = -2635
	self.position.y =  2217
...
func _on_SairdeCasa_TP_body_entered(body):
